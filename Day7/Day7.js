...
let andGate = (string) => {
    ...
    return { dependencies: [input1, input2],
             defined: variable,
             eval: 'var ' + variable + ' = ' + input1 + ' & ' + input2 + ';'
           };
};
...
let notGate = (string) => {
    ...
    return { dependencies: [input],
             defined: variable,
             eval: 'var ' + variable + ' = ~ ' + input + ';'
           };
};

// calls appropriate function depending on type of gate
...
//sort array of objects so that variable defined always comes after dependencies are defined
// 1. go through array element by element
// 2. if current element depends on nothing, leave it
// 3. if it depends on an item after it, swap them (item after it is in current dependences)
// 4. if it depends on another item after that, swap them
// 5. start over upon reaching end
// 6. go until nothing remains to be swapped
...
//do a bubble sort
    ...
    DOESNT WORK:
    let sorted = _.reduce(unsortedCircuits, (result, element, index, array) => {
        return _.transform(result, (subResult, subElement, subIndex, subArray) => {
            if ( subIndex > index && element.dependencies.indexOf(subElement.defined) > -1 ){ //swap them:
                subResult[index] = subElement;
                subResult[subIndex] = element;
            };
        }, result);
    }, unsortedCircuits); 

    if ( _.isEqual(sorted, unsortedCircuits) ) { return sorted; }
    else { return sorter(sorted); };
};
*/
...
// go through, evaluate what you can. then go through again, evaluate what you can, until you can evaluate everything and a is defined

let toDeclare = _.transform(circuits, (result, element) => {
    result.push('var ' + element.defined + ';');
}, []);

// PROBLEM WITH THIS FOR LOOP!
for (let index = 0; index < toDeclare.length; index++) {
    eval(toDeclare[index]);
};

while (true) {
    for (let index = 0; index < circuits.length; index++) {
        //check if dependencies are undefined. if not, evaluate element
        if ( _.transform(circuits[index].dependencies, (result, element) => {
            if (typeof eval(element) == 'undefined') { result = false; };
        }, true) ) { eval(circuits[index].eval) };    
    };
