///make an array with 1000 elements, each element representing a column
//each element is an array of 1000 elements, containing true or false, on or off

"use strict";
let _ = require('lodash');

// create grid of coordinates for the lights, all off
...
let grid = _.times(1000, () => { return _.times(1000, _.constant(false))});

// replace cleanup with regexp?
...
let finalLights = _.reduce(input, (result, instruction) => {
    if (instruction.startsWith('turn on')) { return turnOn(instruction, result); }
    else if (instruction.startsWith('turn off')) { return turnOff(instruction, result); }
    else { return toggle(instruction, result); };    
}, grid);

// getting 537821 but the number is too low. the count is likely correct (got it with for loops) so the problem is above? But its' workign when i test it

let lightsOn = _.reduce(finalLights, (result, column) => {
    return _.reduce(column, (innerResult, element) => {
        if (element) { return innerResult + 1; }
